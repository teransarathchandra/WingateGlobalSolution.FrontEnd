...
import {
  ...
  ImageContainer,
  PrimaryButton,
} from "@app_styles/shared/commonStyles.style";
...
const ShipmentDetailsForm = ({ goNext, goBack }) => {
  const [shipmentDetails, setShipmentDetails] = useSessionStorage(
  ...
  const [itemSubmitted, setItemSubmitted] = useSessionStorage(
    "order-item-submitted"
  );
  const [itemObjectId, setItemObjectId] = useSessionStorage(
    "order-item-object-id"
  );
  const [, setItemId] = useSessionStorage("order-item-id");
  const [isPickupOrder, setIsPickupOrder] = useSessionStorage(
    "order-is-pickup-order",
    false
  );
  const [pickupOrderDate, setPickupOrderDate] = useSessionStorage(
    "order-pickup-order-date"
  );
  const [, setRestrictedOrderType] = useSessionStorage(
    "restricted-order-order-type"
  );
  ...
  const onSubmit = async (data) => {
    ...
    if (data.isPickupOrder && data.pickupOrderDate) {
      const { date } = await separateDateTime(
        data.pickupOrderDate.toISOString(),
        "YYYY-MM-DD"
      );
    ...
    <StyledForm onSubmit={handleSubmit(onSubmit)} width="400px">
      <HeadingSection>
        ...
        >
          ...
          <ImageContainer src={LorryImg} alt="" width="40px" height="40px" />
        </FlexRow>
      ...
      >
        ...
        <Controller
          ...
          render={({ field }) => (
            ...
            >
              ...
              {packageTypes.map((packageType) => (
                <MenuItem key={packageType._id} value={packageType._id}>
        ...
        />
        <FormHelperText error={!!errors.packageTypeId}>
