...
const restrictedOrdersYupSchema = Yup.object().shape({
  ...
  maxQuantity: Yup.number().nullable().typeError("Enter some value").required('Maximum quantity is required.').positive('Maximum quantity must be a positive number.').integer('Maximum quantity must be an integer.'),
